...
Jobs_in_Multi_Sports_cash: Dict[str, str] = {}
test4_2018_with_nat_cash: Dict[str, str] = {}


def nat_match(cate: str, out: bool = False, fa: str = "", tab: Optional[Dict[str, Any]] = None) -> str:
...
def test4_2018_with_nat(cate: str, out: bool = False, fa: str = "", tab: Optional[Dict[str, Any]] = None) -> str:
    if not tab:
        tab = {}
    ...
    if con_3:
        ...
        if not contry_lab:
            contry_lab = nat_match(cate, out=out, fa=fa, tab=tab)
    if not contry_lab:
    ...
    if contry_lab:
        if con_3:
            output_test4(f'<<lightblue>> test4_2018_with_nat con_3:"{con_3}"')
        output_test4(f'<<lightblue>> test_4: test4_2018_with_nat :: "{contry_lab}" ')
    test4_2018_with_nat_cash[cate] = contry_lab
    return contry_lab


def Jobs_in_Multi_Sports(cate: str, out: bool = False, tab: Optional[Dict[str, Any]] = None) -> str:
    ...
    job_lab = ""
    game_lab = ""
    for ga, g_lab in Multi_sport_for_Jobs.items():
        game = f"{ga} "
