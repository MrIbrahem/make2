...
def make_lab_dodo(_category_, Tita_year, tita, tita_other, category3, category, cat_test, category_r, cash_key):
    # ---
    Add_In_Done = False
    NoLab = False
    # ---
    ...
    if not ar_label:
        ...
        if category_lab:
            if re.sub(en_literes, "", category_lab, flags=re.IGNORECASE) == category_lab:
                ...
                return category_lab
        # ---
        ar_label = make_lab_dodo(_category_, Tita_year, tita, tita_other, category3, category, cat_test, category_r, cash_key)
    # ---
    if not ar_label:
