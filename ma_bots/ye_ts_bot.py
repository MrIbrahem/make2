...
def Get_team_work_Club(s):
    ...
    slab = team_work.Get_Club(s, return_tab=True)
...
def find_ar_label(category, tito, tito_name, cash_key, Cate_test, category_r, do_Get_contry2=True):
    # Keep_Work = False
    # NoLabb = True
    CAO = True
    ...
    if Type_lab == "" and Type_lower.strip().endswith(" people"):
        Type_lab = fax.make_people_lab(Type_lower)
    ...
    if not Type_lab:
        Type_lab = Get_team_work_Club(Type.strip())
    if not Type_lab:
    ...
    if not con_lab:
        con_lab = Get_team_work_Club(contry.strip())
    if not con_lab:
...
def yementest_with_Titose_Nmaes(category_r, do_Get_contry2=True):
    ...
    c_t_lab = pop_All_2018.get(category, "")

    if c_t_lab:
        if re.sub(en_literes, "", c_t_lab, flags=re.IGNORECASE) == c_t_lab:
            c_t_lab = fixtitle.fixlab(c_t_lab, en=category_r)
            print_put('>>>>>> <<lightyellow>>Cate_test: "%s" ' % Cate_test)
            print_put(f'>>>>>> <<lightyellow>>test: cat "{category_r}", c_t_lab:"{c_t_lab}"')

            YTN_cash[cash_key] = c_t_lab

            return c_t_lab

    _lab = find_lab(category, category_r, cash_key)
    if _lab:
        print_put("<<lightblue>>>> =================  yementest end 2 =============== ")
        return _lab
    ...
    for tito, tito_name in Tit_ose_Nmaes.items():
        ...
        if category.find(tito) == -1:
            continue
        # ---
        arlabel = find_ar_label(category, tito, tito_name, cash_key, Cate_test, category_r, do_Get_contry2=do_Get_contry2)
        # ---
        if arlabel:
    ...
    print_def_head("<<lightyellow>>>> ^^^^^^^^^ yementest end 3 ^^^^^^^^^ ")
...
def find_lab(category, category_r, cash_key):
    ...
    if _lab:
        if re.sub(en_literes, "", _lab, flags=re.IGNORECASE) == _lab:
